...
#define CHARA_HEALTHGAUGE_HIDE_DELAY				(10.0f)
...
typedef struct _SCharaInfo
{
	...
	// アニメーションの移動値ではなく、MoveSpeed による移動を行うかどうかのフラグ
	bool                  ProgramMove;

	// 移動速度
	VECTOR                MoveSpeed;

	// 一つ前のフレームでの MoveSpeed の値
	VECTOR                PrevMoveSpeed;

	// 一つ前のフレームでのルートフレームのアニメーションによる移動速度
	VECTOR                PrevAnimMoveSpeed;

	// 一つ前のフレームでの最終的な移動速度
	// ( MoveSpeed にアニメーションの移動値などを加算したもの )
	VECTOR                PrevFixMoveSpeed;
